...
## The tests are kept in the directory Data/Demo/Python (for this one), in python files.
...
class PythonAcceptanceTest(AcceptanceTestHelpers.AcceptanceTestHelpers):
...
###############################################################
    ...
    def test_make_new_acceptance_test(self):
        """define new statements for a python acceptance test
        """
## instructions:        
...
## In Data/Demo/python a python file "name.py" is created, which contains the phrases and
## window contents.
...
## 
        ...
        source =    \
"""        
    commands.say(['define', 'class', 'some', 'variable', 'class', 'body'])
    commands.say(['define', 'method', 'some', 'method', 'add', 'arguments'])
    commands.say(['collect', 'arguments', 'positional', 'arguments', 'comma'])
    commands.say(['collect', 'keyword', 'arguments', 'keyword', 'arguments'])
    commands.say(['class', 'body'])
    commands.say(['some', 'array', 'equals', 'some', 'other', 'array','sliced', 'at', 'one', 'colon', 'five', 'new', 'statement'])
    commands.say(['some', 'dictionary', 'item', 'with', 'key', 'zero', 'jump', 'out', 'equals', 'one'])
    commands.say(['comment', 'above'])
    commands.say(['this', 'is', 'a', 'commented', 'out'])
"""
