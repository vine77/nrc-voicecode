;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;
;; Old version of vc.el which used overlays as per Barry Jaspan's vr.el
;;
;; We now use after-change-functions which is much simpler and more robust
;;;
;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;
...
Argument 'change-list is a list of 4ple:
   (buffer-name inserted-start inserted-text deleted-length).
"
  ...
  (let ((mess "") (buff-name) (a-change) (inserted-start) (inserted-end) 
    ...
    (while change-list
      ...
      (setq inserted-start (nth 1 a-change))
      (setq inserted-text (nth 2 a-change))
      (setq deleted-length (nth 3 a-change))

      (setq deleted-start inserted-start)
      (setq deleted-end (+ deleted-start deleted-length))

      (vr-log "**-- vcode-serialize-changes: serializing a-change=%S\n, buff-name=%S, inserted-start=%S, deleted-length=%S\n" a-change buff-name inserted-start deleted-length)
      ...
      (if (vr-change-is-delete deleted-start deleted-end 
			       (length inserted-text))
	  (cl-puthash "action" "delete" a-change-vcode)
	(cl-puthash "action" "insert" a-change-vcode)
	(cl-puthash "text" inserted-text a-change-vcode)
      )
